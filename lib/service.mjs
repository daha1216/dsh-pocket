...
export function createPocketService({
  ...
  injectHtml,
  /** 访问令牌认证配置（issue #13）：{ getToken, isProtected, scopeFor, getTokenForScope }，传给代理 */
  auth,
  /** 设备注册表（lib/devices.mjs 的 store）：传给代理做 v2 设备 cookie 与活 WS 归属 */
  devices,
  ...
  return {
      ...
      for (let p = port; p < port + 10; p++) {
        try {
          proxy = await createProxy({
            ...
            ...(auth ? { auth } : {}),
            // 设备注册表（本轮新增）：v2 设备 cookie / 单独下线 / 活 WS 归属
            ...(devices ? { devices } : {}),
    ...
    /** 当前公网隧道地址（同步；未开启返回 null）。VAPID sub 推导用（见 lib/index.js）——
     *  快速隧道的 trycloudflare 域名只有 cloudflared 起来后才知道，只能运行时取。 */
    tunnelUrl() {
