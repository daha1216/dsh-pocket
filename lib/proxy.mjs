 ...
 *
 * R7（同一个 shim，第二件事）：`ownsHost`——让「经代理访问」的页面被连接层认作
 * 「本机直连」，修手机端设置 → 插件 → 插件配置子类永远空白。
 * 根因在宿主：packages/client/connection/src/client/index.ts
 *   isLoopback = transport?.ownsHost === true || pageLocation === undefined
 *                 || isLoopbackHostname(pageLocation.hostname)
 * 手机经公网隧道域名 / 局域网 IP 访问时 hostname 非 loopback → isLoopback=false →
 * ui-settings 的 describe 镜像 persistence='memory'（远端不可读）→ 镜像 status 停在
 * 'unavailable'，ensure() 直接返回、**永不发 settings/describe 读** → 插件配置 tab 的
 * namespaces 永远空（用户可见症状：插件配置子类一片空白）。
 * 这里把 ownsHost 补成 true：isLoopback=true → persistence='host' → describe 正常加载，
 * 插件配置卡片出来；ui-settings-general 的 SettingsDocumentStore（「打开配置文件」）
 * 也随之点亮。两者都是正向收益，且经代理访问本来就要过 PIN 门禁，权限语义正确。
 *
 * 保守规则（绝不覆盖宿主的真实现）：
 *   - `__DSH_TRANSPORT__` 不存在 → 建 `{ ownsHost: true }`（再走下面的 patch 补
 *     createApiClient；只给 ownsHost 不补 createApiClient 会让
 *     `transport?.createApiClient()` 变 TypeError，issue #96 原地复发）；
 *   - 已存在 → 仅当 `!('ownsHost' in t)` 时补 t.ownsHost = true，其余字段一律不动；
 *   - 每次给全局赋值（setter）走同一个 patch，语义一致。
 * 注：不加 rpc/fetch/openStream 是**刻意**的——connection 里
 * `rpc = transport?.rpc ?? createWebConnectionRpc(transport?.fetch, transport?.openStream)`，
 * 三者都缺时 createWebConnectionRpc 内 `doFetch ?? ((input, init) => globalThis.fetch(...))`
 * 兜底到页面全局 fetch，浏览器 HTTP/WS 载体不受影响。
 *
 * 边界：本 shim 只在**代理响应链**注入（DEFAULT_INJECT → createPocketProxy 的
 * injectHtml），3080 直连桌面端不经过这里，行为不变。
 */
export const TRANSPORT_API_CLIENT_SHIM = `<script data-dsh-pocket-transport-shim="1" data-dsh-pocket-owns-host="1">!function(){try{var K='__DSH_TRANSPORT__',cur=globalThis[K];function patch(t){try{if(t&&typeof t==='object'){if(!('ownsHost' in t)){try{Object.defineProperty(t,'ownsHost',{value:true,writable:true,configurable:true,enumerable:true});}catch(e2){try{t.ownsHost=true;}catch(e3){}}}if(typeof t.createApiClient!=='function'){try{Object.defineProperty(t,'createApiClient',{value:function(){return null;},writable:true,configurable:true});}catch(e4){try{t.createApiClient=function(){return null;};}catch(e5){}}}}}catch(e){}return t;}if(cur){patch(cur);}else{cur=patch({ownsHost:true});}Object.defineProperty(globalThis,K,{configurable:true,enumerable:true,get:function(){return cur;},set:function(t){cur=patch(t);}});}catch(e){}}();</script>`;
