  ...
  h.factory=function(r){
    ...
    if(exp&&typeof exp.apply==='function'){
      ...
      exp.apply=function(ctx){
        var out=oa.apply(this,arguments);
        function forceLoopback(){
          try{
            var v=ctx&&typeof ctx.get==='function'?ctx.get('connection',false):null;
        ...
        if(out&&typeof out.then==='function'){
          return out.then(function(v){forceLoopback();return v;});
        }
        forceLoopback();
        return out;
      };
...
/** 默认注入到经代理的 HTML 文档里：仅保留 polyfill。
 * LOOPBACK_ENV_PATCH 已停用（2026-09-05）：新版 DSH 前端 bundle（rev=28e1db08b1d0）下，
 * 该补丁对 __ModuleLoader__/facade.load 的劫持会使 connection 服务不可用，应用卡在
 * Loading plugins、会话内容不显示（无头 Chromium 经隧道实测复现，剥离该补丁即恢复）。
 * 且新版前端 bundle 与 client-modules 中 isLoopback 出现 0 次——issue #58 场景已不存在。
 * 如 dsh 前端重新出现 loopback 判定问题，把 LOOPBACK_ENV_PATCH 加回拼接即可。 */
export const DEFAULT_INJECT = RANDOM_UUID_POLYFILL;

/**
 * 手机诊断探针（2026-09-05 临时，定位 iPhone Safari「会话列表在、对话内容空白/一直加载」）。
 * 注入到 <head> 最前、先于所有应用脚本执行；采集 onerror / unhandledrejection /
 * console.error / remote.mux WebSocket 事件 / [data-phase] 变化与转录文本长度，
 * 每 8 秒 POST /pocket-diag 上报（同源 fetch，服务端落 phone-diag.log）。
 * 探针自身全程 try/catch，任何异常不得影响页面；上报上限 40 次防刷。
 */
export const PHONE_DIAG_SCRIPT = `<script data-dsh-pocket-diag="1">!function(){try{
if(window.__dshPocketDiag)return;
var D=window.__dshPocketDiag={t0:Date.now(),id:Math.random().toString(36).slice(2),events:[],sent:0,lastScroll:-1,phases:[]};
function txt(x){try{if(x&&x.stack)return String(x.stack).slice(0,400);if(x&&x.message)return String(x.message).slice(0,400);return String(x).slice(0,400);}catch(e){return "txt-err"}}
function ev(k,d){try{D.events.push({t:Date.now()-D.t0,k:k,d:txt(d)});if(D.events.length>150)D.events.shift();}catch(e){}}
window.addEventListener("error",function(e){ev("onerror",(e.message||"")+" @"+(e.filename||"")+":"+(e.lineno||0)+" :: "+(e.error?txt(e.error):""))},true);
window.addEventListener("unhandledrejection",function(e){ev("rejection",e.reason?txt(e.reason):"no-reason")});
var oc=null;try{oc=console.error;console.error=function(){try{ev("console.error",Array.prototype.map.call(arguments,function(a){return a&&a.stack?txt(a):String(a).slice(0,200)}).join(" | ").slice(0,500))}catch(e){}return oc.apply(console,arguments)};}catch(e){}
var OW=null;try{OW=window.WebSocket;}catch(e){}
if(OW){try{
function DW(url,protos){var ws;try{ws=protos===undefined?new OW(url):new OW(url,protos);}catch(err){throw err;}
try{if(String(url).indexOf("remote.mux")!==-1){ev("ws-new",url);ws.addEventListener("open",function(){ev("ws-open","")});ws.addEventListener("close",function(c){ev("ws-close","code="+c.code+" reason="+String(c.reason).slice(0,120))});ws.addEventListener("error",function(){ev("ws-error","")});}}catch(e){}
return ws;}
DW.prototype=OW.prototype;try{DW.CONNECTING=OW.CONNECTING;DW.OPEN=OW.OPEN;DW.CLOSING=OW.CLOSING;DW.CLOSED=OW.CLOSED;}catch(e){}
window.WebSocket=DW;
}catch(e){}}
function ls(k){try{return localStorage.getItem(k)}catch(e){return "LS-ERR"}}
function poll(){try{
var ph=document.querySelector("[data-phase]");
if(ph){var p=ph.getAttribute("data-phase");if(D.phases[D.phases.length-1]!==p){D.phases.push(p);ev("phase",p);}}
var sc=document.querySelector("[data-conversation-scroll]");
if(sc){var n=(sc.innerText||"").length;if(n!==D.lastScroll){D.lastScroll=n;ev("scroll-len",String(n));}}
var boots=document.querySelectorAll("script[src]").length;if(D.bootScripts===undefined)D.bootScripts=boots;
var ml=window.__ModuleLoader__;var mls=ml?String(ml.mode):"none";if(mls!==D.mlMode){D.mlMode=mls;ev("loader-mode",mls);}
}catch(e){}}
try{setInterval(poll,2000);if(document.body)poll();else document.addEventListener("DOMContentLoaded",poll);}catch(e){}
function send(){if(D.sent>40)return;D.sent++;try{
var payload={id:D.id,t0:D.t0,now:Date.now(),ua:navigator.userAgent,sw:screen.width+"x"+screen.height,dpr:window.devicePixelRatio,
cur:ls("dsh.sessions.current"),layout:ls("dsh-pocket.layout"),phases:D.phases,lastScroll:D.lastScroll,events:D.events.slice(-80)};
fetch("/pocket-diag",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)}).catch(function(){});
}catch(e){}}
try{setInterval(send,8000);setTimeout(send,5000);setTimeout(send,15000);
document.addEventListener("visibilitychange",function(){if(document.visibilityState==="hidden")send()});
window.addEventListener("pagehide",function(){send()});}catch(e){}
}catch(e){}}();</script>`;

/** 实际注入：polyfill + 诊断探针（探针定位完成后移除）。 */
export const DEFAULT_INJECT_WITH_DIAG = DEFAULT_INJECT + PHONE_DIAG_SCRIPT;

/** 诊断日志路径：<DSH_HOME>/dsh-pocket/phone-diag.log。 */
const DIAG_LOG_PATH = (() => {
  try {
    const home = process.env.DSH_HOME || join(homedir(), '.dsh');
    return join(home, 'dsh-pocket', 'phone-diag.log');
  } catch {
    return null;
  }
})();
 ...
 *   - 来源是 loopback（cloudflared 隧道回连，源地址必为 127.0.0.1）→ 认 cf-connecting-ip
 *     （Cloudflare 边缘写的真实客户端 IP）；
 *   - 其余一切来源 → 一律用 socket 源地址，**不看任何请求头**。
 *
 * 为什么不能无条件信任 cf-connecting-ip：那是**请求头**，不是可信信道。能直连代理端口
 * （局域网 IP、或你自己暴露出去的端口）的人只要每次请求换一个头值，就等于每发一个请求
 * 换一个限速身份，8 位 PIN 的「连续输错 5 次锁 60 秒」完全失效，退化成可全速穷举。
 * 隧道流量必经本机 cloudflared（源地址必为 loopback），所以把采信范围限定在 loopback
 * 既不影响隧道场景，又让远程来源「换头即换身份」的限速绕过失效。XFF 同理，始终不认。
 *
 * 已知边界：本机进程仍可伪造该头换身份。它已经有本机任意执行权限，不是这条防线要挡的
 * 对象（详见 SECURITY-NOTES.md）。
 */
...
function loginPageHtml(error, isPublic, retryAfter = 0, returnQuery = '') {
...
<form method="post" action="/pocket-login${returnQuery}">
...
export function isProtectedHost(host, isProtected) {
...
// ---------- 首登参数保留（2026-09-06） ----------
// 用户带 ?dsh-layout=desktop/mobile（或任何业务参数）首次打开公网地址时，登录链路
// 「密码页 → POST /pocket-login → 302 → 握手过渡页 → meta refresh 回 /」原本会把查询串
// 一路丢光：客户端 layout-mode.mjs 只有页面真正加载后才能读 ?dsh-layout=，而首登时它
// 还没机会运行、localStorage 也还是空的 → 参数静默失效。下面两个函数把「非 pocket 自用」
// 的参数挑出来（URLSearchParams 重新编码，可安全内插进 HTML 属性与 Location 头），
// 让登录链路的每一跳都带着走。
export function preservedQuery(reqUrl) {
  let u;
  try { u = new URL(reqUrl ?? '/', 'http://dsh.invalid'); } catch { return ''; }
  const p = new URLSearchParams();
  for (const [k, v] of u.searchParams) {
    if (k === 'token' || k === 'dsh-pocket-auth' || k === HANDSHAKE_RETRY_PARAM || k.startsWith('dsh-desktop-')) continue;
    p.append(k, v);
  }
  const s = p.toString();
  return s ? `?${s}` : '';
...
/** 过渡页回跳目标：原请求去掉 pocket 自用参数后的干净 URL；解析失败或可疑目标兜底 '/'。 */
export function handshakeRefreshTarget(reqUrl) {
  let u;
  try { u = new URL(reqUrl ?? '/', 'http://dsh.invalid'); } catch { return '/'; }
  const target = u.pathname + preservedQuery(reqUrl);
  // 只允许站内相对目标：拒绝协议相对 //host 与反斜杠混淆（meta refresh 开放重定向防护）
...
/** 握手过渡页：200 + Set-Cookie（由调用方带上）+ meta refresh 跳回干净根路径（保留 dsh-layout 等业务参数）。 */
export function handshakePageHtml(target = '/') {
  const safe = typeof target === 'string' && target.startsWith('/') && !target.startsWith('//') && !target.includes('\\') ? target : '/';
  return `<!doctype html><html lang="zh"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
...
export function handshakeBlockedPageHtml(retryQuery = '') {
  return `<!doctype html><html lang="zh"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
...
<p style="margin-top:14px"><a href="/?${HANDSHAKE_RETRY_PARAM}=1${retryQuery ? '&' + retryQuery.slice(1) : ''}" style="display:inline-block;padding:8px 14px;background:#4f6ef7;color:#fff;border-radius:8px;text-decoration:none;font-size:13px">重试一次 | Retry</a></p>
 ...
 */
export function createPocketProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT_WITH_DIAG, auth = null, rateLimit = null, heartbeat = {}, lanAccessEnabled = () => true, launchToken = () => '', handshakeLimit } = {}) {
  ...
  const server = createServer((req, res) => {
    ...
    // 手机诊断上报（2026-09-05 临时）：PHONE_DIAG_SCRIPT 的落点。置於认证之前——
    // 即使 cookie 状态坏了也能收到报告。仅追加 ≤2MB 日志，满了停写。
    if (req.method === 'POST' && req.url?.startsWith('/pocket-diag')) {
      let body = '';
      req.on('data', (c) => { body += c; if (body.length > 96 * 1024) req.destroy(); });
      req.on('end', () => {
        try {
          if (DIAG_LOG_PATH !== null) {
            let writable = true;
            try { writable = !existsSync(DIAG_LOG_PATH) || statSync(DIAG_LOG_PATH).size < 2 * 1024 * 1024; } catch { writable = true; }
            if (writable) appendFileSync(DIAG_LOG_PATH, `${new Date().toISOString()} ${clientIp(req)} ${body}\n`);
          }
        } catch { /* 诊断失败不影响页面 */ }
      ...
      if (protectedHost && token) {
        ...
        if (req.method === 'POST' && req.url?.startsWith('/pocket-login')) {
          ...
          if (rl.locked) {
              ...
              res.end(loginPageHtml('locked', isPublic, rl.retryAfter, preservedQuery(req.url)));
            return;
          }
          let body = '';
          ...
          req.on('end', () => {
            ...
            if (matched !== undefined) {
              limiter?.clear(ip);
              // 表单 action 带回的原始参数（如 ?dsh-layout=desktop）：跨登录全程保留，
              // 否则客户端 layout-mode.mjs 在首登时永远读不到它（页面还没加载过）
              const retQ = preservedQuery(req.url);
              res.writeHead(302, {
                // 带上 dsh-pocket-auth=1：登录成功后强制重做一次浏览器会话握手，
                // 换掉可能已过期/被撤销的 dsh web 会话 cookie（issue #77）
                location: '/?dsh-pocket-auth=1' + (retQ ? '&' + retQ.slice(1) : ''),
              ...
              res.end(loginPageHtml(true, isPublic, 0, preservedQuery(req.url)));
            }
          });
          return;
        }
        ...
        if (!authResult.ok) {
          ...
          if (isHtmlRequest(req)) {
            ...
            const rl = limiter?.status(ip) ?? { locked: false, retryAfter: 0 };
            res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
            res.end(loginPageHtml(rl.locked ? 'locked' : false, isPublic, rl.retryAfter, preservedQuery(req.url)));
          } else {
    ...
    if (handshakeOver && isHtmlRequest(req)) {
      ...
      res.end(handshakeBlockedPageHtml(preservedQuery(req.url)));
      return;
    }
    const proxyReq = httpRequest(
      ...
      (proxyRes) => {
        ...
        if (didInjectToken && proxyRes.statusCode === 303 && isHtmlRequest(req)) {
          ...
          const page = Buffer.from(handshakePageHtml(handshakeRefreshTarget(req.url)), 'utf8');
        ...
        // 401 自愈（2026-09-05）：浏览器持有「有效 pocket cookie + 失效 dsh-auth-* cookie」
        // 时，门禁放行、上游拒绝 → 裸 401 且无任何恢复路径（正常密码页被 pocket cookie 短路）。
        // 对 HTML 导航请求重定向到强制握手参数，重新签发 dsh-auth；循环由既有 handshake
        // tracker（60s 内 3 次上限）与「已带参数则不再重定向」双保险兜底。
        if (proxyRes.statusCode === 401 && !res.headersSent
          && (req.method === 'GET' || req.method === 'HEAD')
          && isHtmlRequest(req)
          && String(req.headers.cookie ?? '').includes('dsh-auth-')
          && !String(req.url ?? '').includes('dsh-pocket-auth')) {
          const target = String(req.url || '/') + (String(req.url).includes('?') ? '&' : '?') + 'dsh-pocket-auth=1';
          log?.(`401 self-heal: stale dsh-auth cookie on ${String(req.url).slice(0, 80)} -> redirect ${target.slice(0, 90)}`);
