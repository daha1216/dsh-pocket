...
import { advancedNoticeScript, DEFAULT_INJECT, classifyHost, identifyDeviceFromCookie, authCookieHeader, deviceCookieValue, pwaAsset, POCKET_ASSETS_PREFIX } from './proxy.mjs';
import { createDeviceStore } from './devices.mjs';
import { createPushNotifier, vapidPath, vapidSubjectFrom } from './webpush.mjs';
import { lanEnabled, setLanEnabled, lanAuthEnabled, setLanAuthEnabled, lanIpOverride, setLanIpOverride, pinCustom, setPinCustom, tunnelMode, setTunnelMode, tunnelToken, setTunnelToken, tunnelHostname, setTunnelHostname, resetSettings, proxyPort, cloudflaredPath, disclaimerMode, setDisclaimerMode, disclaimerAckedAt, ackDisclaimer } from './settings.mjs';
...
/**
 * R7：把 PWA 静态资产（manifest / icon-192 / icon-512 / sw.js）同时挂到 **dsh web 本体
 * （3080）** 上。
 *
 * 为什么：这四个资产原先只有代理（3081）自己的路由（proxy.mjs 的 pwaAsset()，免认证）。
 * 桌面浏览器直连 3080 时点「开启通知」→ enableNotify 执行
 *   navigator.serviceWorker.register('/dsh-pocket-assets/sw.js', { scope: '/' })
 * 直接 404（用户报错原文）：
 *   Failed to register a ServiceWorker for scope ('http://127.0.0.1:3080/')
 *   with script ('http://127.0.0.1:3080/dsh-pocket-assets/sw.js'): 404
 * 桌面直连 3080 的就是本机用户（本来拥有完全权限），资产内容也只是品牌 manifest +
 * PNG 图标 + SW 脚本（无任何会话数据、无 PIN），所以这里补一条公开静态路由。
 *
 * 认证策略：与 3081 侧**完全一致**——公开资产，不做 requestRejection。
 * 理由：① SW 注册请求由浏览器自己发起，凭据语义与 cookie 无关，加认证栅栏只会让注册
 * 变成另一类失败（401/403），症状不消失；② manifest/icon 抓取按规范凭据模式为 omit；
 * ③ 内容无敏感信息。这与 3081 上「PWA 资产放在访问密码认证之前」是同一套语义。
 *
 * 生命周期：与 /dsh-pocket RPC 路由同一套——webServer.register 的返回值规整成**幂等**
 * disposer（照抄 web-rpc.js mountPocketWebRoute 的归一化模式），再随插件的 ctx.effect
 * 清理（apply() 末尾那个 effect 会把 disposers 逆序清掉）。
 *
 * 路由前缀 /dsh-pocket-assets 与 /dsh-pocket 是两个独立前缀（webserver 是
 * longest-prefix-wins 匹配：pathname === prefix || startsWith(prefix + '/')），互不冲突。
 *
 * @returns {(() => void) | null} 幂等 disposer；null 表示环境无 webServer。
 */
function mountPocketAssetRoute(ctx, { log } = {}) {
  const webServer = ctx?.webServer;
  if (!webServer || typeof webServer.register !== 'function') return null;
  const route = {
    kind: 'prefix',
    path: POCKET_ASSETS_PREFIX,
    handler: (req, res) => {
      // 只放行 GET/HEAD（与 3081 侧一致）：资产没有写语义，其它方法 405。
      if (req.method !== 'GET' && req.method !== 'HEAD') {
        res.writeHead(405, { 'content-type': 'text/plain; charset=utf-8', allow: 'GET, HEAD' });
        res.end('method not allowed');
        return;
      }
      const asset = pwaAsset(req.url);
      if (!asset) {
      ...
      res.writeHead(200, {
        ...
        // 与代理侧同款标记（便于端到端核验这四个响应确实来自本插件）
        'x-dsh-pocket-pwa': '1',
        ...(asset.headers ?? {}), // Service Worker 的 Service-Worker-Allowed: /
      });
  ...
  // 统一规整成「可幂等调用的清理函数」（与 web-rpc.js 同一套）。
  const cleanup = typeof registered === 'function'
    ...
    : (registered && typeof registered.then === 'function')
      ...
      : () => {};
  log?.info?.('dsh-pocket: PWA assets mounted on the host web server (direct 3080 access) | 本体 3080 已挂载 PWA 资产路由');
  return cleanup;
}
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  // 设备注册表（本轮新增）：v2 登录 cookie 的设备身份 + 设置页「已授权设备」列表。
  // 由 lib/index.js 单例持有，同时交给代理（校验/断开活 WS）与 RPC 层（列表/改名/下线）。
  const devices = createDeviceStore({ log: (m) => logger.warn?.(m) ?? logger.info?.(m) });
  // 会话密钥（issue #33）：进程级随机——登录 cookie 绑定它，dsh web 重启后所有 cookie 失效。
  // v2 设备 cookie 的 hash 也把它算进公式（同一条会话保持语义）。
  const sessionKey = randomBytes(16).toString('hex');
  /** 某一侧（lan/public）当前的 PIN：v2 cookie 校验与 legacy 比对都走它。 */
  const pinForScope = (scope) => (scope === 'public' ? getAccessToken() : getLanToken());
  /** 该 Host 属于哪一侧 PIN 边界（与 tokenForHost 同源，保证 scope 与 PIN 对得上）。 */
  const scopeForHost = (host) => (isLanOverrideHost(host) || classifyHost(host) !== 'public' ? 'lan' : 'public');
  ...
  const service = internals.service ?? createPocketService({
    ...
    auth: {
      ...
      // 设备 cookie（v2）用：按 scope 取当前 PIN + 按 Host 归一化 scope
      getTokenForScope: (scope) => pinForScope(scope),
      scopeFor: (host) => scopeForHost(host),
      isProtected: (host) => {
  ...
  const disposers = [];
  // Web Push 通知（R3）：事件通知器 —— VAPID 密钥（首次发送时生成 ~/.dsh/dsh-pocket/vapid.json）、
  // RFC 8291 加密发送、5 秒防抖、活 WS 抑制。RPC 侧只用它的 vapidKey()/test()。
  const notifier = createPushNotifier({
    devices,
    file: internals.vapidFile ?? vapidPath(internals.home),
    log: (m) => logger.warn?.(m) ?? logger.info?.(m),
    // VAPID sub（RFC 8292）：**每次发送时**动态解析（Apple 拒绝一切含 localhost 的 sub，
    // 403 BadJwtToken，见 lib/webpush.mjs 的 VAPID_SUBJECT 注释）：
    //   ① 命名隧道固定域名（settings.json tunnelHostname）→ https://<域名>
    //   ② 快速隧道当前地址（cloudflared 报的 trycloudflare 域名，运行时才有）
    //   ③ 兜底 mailto:dsh-pocket@invalid（不含 localhost 的合法形态）
    subject: () => vapidSubjectFrom({ hostname: tunnelHostname(), tunnelUrl: service.tunnelUrl?.() ?? '' }),
    ...(internals.pushNotifierOptions ?? {}),
  });
  // 事件通路：与 dsh-pet host 侧**完全相同**的一条（ctx.on('session/event', ...)，见
  // node_modules/dsh-pet/src/host/index.ts:856）——它消费同一批事件做工作状态联动 +
  // 浏览器系统通知；我们只取两种「需要人回来」的时刻：turn/end(completed) 与 approval/asked。
  ctx.effect(() => {
    const dispose = ctx.on('session/event', (session, event) => {
      try {
        notifier.handleSessionEvent(session, event);
      } catch (err) {
        logger.warn?.('dsh-pocket: push event handler failed | 推送事件处理失败: %s', err?.message ?? err);
      }
    });
    return () => { try { dispose?.(); } catch { /* 忽略 */ } };
  }, 'dsh-pocket: web push session events');
  const disposeRpc = installPocketRpc(ctx, {
    service,
    desktop: isDesktop,
    // 已授权设备（本轮新增）：列表/改名/单独下线/全部下线
    devices,
    // 推送通知（R3）：notify.vapidKey 返回 VAPID 公钥（base64url），notify.test 真发一条
    notify: {
      vapidKey: () => notifier.vapidKeys().publicKey,
      test: (deviceId) => notifier.sendTest(deviceId),
    },
    // RPC 请求自己的设备身份（标记「本机」）：从 /dsh-pocket 请求头的 dsh_pocket_token 解析。
    // 走的是与代理完全同一套校验（hash 重算 + 注册表查表），不做猜测。
    identifyDevice: (cookieHeader) => identifyDeviceFromCookie(cookieHeader, { devices, sessionKey, pinForScope }),
    // 「下线其他设备」会把 epoch +1：调用者那枚 cookie 的 hash 也随之失效，
    // 所以由这里按新 epoch 给他重算一枚（Set-Cookie 由 RPC 响应带回浏览器）。
    refreshCallerCookie: (deviceId) => {
      const d = devices.get(deviceId);
      const pin = d ? pinForScope(d.scope) : '';
      if (!d || !pin) return null;
      return authCookieHeader(deviceCookieValue(d.id, pin, sessionKey, devices.epoch()));
    },
    ...
    // 安全免责声明（本轮）：tunnel.start 按模式校验（never 不校验 / once 首次确认后记住 / always 每次），
    // 设置页读同一份状态决定弹不弹；disclaimer.setMode 改模式（枚举非法即拒）。
    getDisclaimerMode: () => disclaimerMode(),
    setDisclaimerMode: (mode) => setDisclaimerMode(mode),
    getDisclaimerAckedAt: () => disclaimerAckedAt(),
    acknowledgeDisclaimer: () => ackDisclaimer(),
  ...
  // R7：PWA 静态资产同时挂到本体 3080（桌面直连 3080 时 SW 注册不再 404）。
  // 与 RPC 路由同一套生命周期：disposer 进 disposers，由末尾的 ctx.effect 统一清理。
  const disposeAssets = mountPocketAssetRoute(ctx, { log: logger });
  if (disposeAssets) disposers.push(disposeAssets);
