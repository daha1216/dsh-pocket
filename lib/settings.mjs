...
// ---------- 公网开启前的安全免责声明（issue #31；本轮改为可配置 never | once | always） ----------
// 用户要求「不要每次声明、行为可自行配置」：
//   - never ：永不弹，直接开启公网；
//   - once  ：首次开启弹一次（勾选后记住时间戳 disclaimerAckedAt），之后直接开（**默认**）；
//   - always：每次都弹（现行行为）。
// 服务端仍是唯一权威：tunnel.start 按模式校验（见 lib/web-rpc.js），前端只是按同一状态决定弹不弹。
// 恢复出厂设置删除整个 settings.json → 模式回 once、确认状态清空（resetSettings 天然覆盖）。

/** 声明模式：'once'（默认）| 'never' | 'always'；缺失/非法一律按 once。 */
export function disclaimerMode() {
  const v = readSettings().disclaimerMode;
  return v === 'never' || v === 'always' ? v : 'once';
}

/** 设置声明模式（持久化）；非法值抛错。once 是默认值，不落盘（清掉字段）。 */
export function setDisclaimerMode(mode) {
  if (mode !== 'never' && mode !== 'once' && mode !== 'always') {
    throw new Error('声明模式必须是 never/once/always | disclaimer mode must be never, once or always');
  }
  const s = readSettings();
  if (mode === 'once') delete s.disclaimerMode;
  else s.disclaimerMode = mode;
  writeSettings(s);
  return mode;
}

/** 用户确认过声明的时间戳（毫秒；未确认返回 0）。 */
export function disclaimerAckedAt() {
  const v = Number(readSettings().disclaimerAckedAt);
  return Number.isFinite(v) && v > 0 ? Math.floor(v) : 0;
}

/** 记录一次声明确认（返回时间戳）。 */
export function ackDisclaimer(at = Date.now()) {
  const s = readSettings();
  s.disclaimerAckedAt = Math.floor(at);
  writeSettings(s);
  return s.disclaimerAckedAt;
}
