...
import { POCKET_RPC_CHANNEL, POCKET_ENDPOINTS, redactStatus, deviceViews, LEGACY_DEVICE_ID } from '../client/api.js';
...
function pocketFetchHandler(channel, handler, log) {
  return {
    async fetch(request) {
      ...
      try {
        // 第 4 个参数是 Request 本体（本轮新增）：设备相关 endpoint 需要读它自己的
        // dsh_pocket_token 来识别「本机」，旧 endpoint 全部忽略它，行为不变。
        // 第 5 个参数是「响应头出口」：目前只有 devices.revokeOthers 用它回种一枚
        // 新 cookie（epoch 变了，调用者手里的旧 cookie 已失效，必须当场换新）。
        const extra = { headers: {} };
        const result = await handler(endpoint, body.payload, request.signal, request, extra);
...
export function installPocketRpc(ctx, { service, log = console, desktop = false, runUpdate = null, restart = null, restartNotice = null, getToken = null, getLanToken = null, refreshLanToken = null, getLanAuthEnabled = null, setLanAuthEnabled = null, getLanEnabled = null, setLanEnabled = null, getMobileRightbarEnabled = null, setMobileRightbarEnabled = null, getLanIpOverride = null, setLanIpOverride = null, getPinCustom = null, setCustomPin = null, getTunnelConfig = null, setTunnelConfig = null, resetPocket = null, devices = null, identifyDevice = null, refreshCallerCookie = null, notify = null, getDisclaimerMode = null, setDisclaimerMode = null, getDisclaimerAckedAt = null, acknowledgeDisclaimer = null }) {
  const pocketRpcHandler = async (endpoint, payload = {}, signal, request = null, extra = null) => {
    if (signal?.aborted) return fail('cancelled', 'The request was cancelled.');

    /** 调用方自己的设备身份（cookie 解析；识别不出返回 null）。 */
    const currentDevice = () => {
      try { return identifyDevice?.(request?.headers?.get?.('cookie') ?? '') ?? null; } catch { return null; }
    };
    /** 设备列表视图（含 currentId 标记）——deviceViews 在 client/api.js，前后端共用同一份投影。
     *  currentIdOverride：revokeOthers 会在读 payload 之前先把 epoch +1，此时请求里那枚旧 cookie
     *  已经校验不过、currentDevice() 会返回 null；调用方身份是已知的（keep），直接传进来，
     *  否则响应里自己的行会短暂丢掉「本机」标记。 */
    const devicesPayload = (currentIdOverride) => {
      const cur = currentIdOverride === undefined ? currentDevice() : (currentIdOverride ? { id: currentIdOverride } : null);
      return { ...deviceViews(devices, { currentId: cur?.id ?? null }), epoch: devices?.epoch?.() ?? 1 };
    };
    ...
    const statusPayload = async () => {
      ...
      return ok({
        ...
        // 安全免责声明状态（本轮）：模式 + 确认时间戳（0=未确认）。前端据此决定开启公网时弹不弹。
        disclaimerMode: getDisclaimerMode?.() ?? 'once',
        disclaimerAckedAt: getDisclaimerAckedAt?.() ?? 0,
      });
    };
    ...
    try {
      ...
      if (endpoint === POCKET_ENDPOINTS.tunnelStart) {
        // 安全免责声明（issue #31；本轮可配置）：模式来自 settings（never | once | always，默认 once）。
        //   - never ：不弹不校验（用户明确要「不再每次声明」）；
        //   - once  ：首次需 disclaimer:true（服务端记下确认时间戳），之后直接放行；
        //   - always：每次都要 disclaimer:true（现行行为）。
        // 服务端始终是权威（前端弹框只是体验层），防绕过前端直接调 RPC。
        const mode = getDisclaimerMode?.() ?? 'once';
        const ackedAt = getDisclaimerAckedAt?.() ?? 0;
        const acknowledged = payload?.disclaimer === true;
        // 仅在「需要确认的模式」下记录确认：never 模式用户已明确选择不再提示，
        // 此时开启不该偷偷写 disclaimerAckedAt（否则日后切回 once 会一次都不弹）。
        if (acknowledged && mode !== 'never') {
          try { acknowledgeDisclaimer?.(); } catch { /* 记录失败不影响本次放行 */ }
        }
        if (mode === 'always' && !acknowledged) {
          return fail('bad-request', '开启公网前请先阅读并勾选安全免责声明 | please accept the security disclaimer before enabling public access');
        }
        if (mode === 'once' && !ackedAt && !acknowledged) {
          return fail('bad-request', '开启公网前请先阅读并勾选安全免责声明（仅首次需要）| please accept the security disclaimer before enabling public access (first time only)');
        }
        await service.startTunnel();
        return await statusPayload();
      }
      if (endpoint === POCKET_ENDPOINTS.disclaimerSetMode) {
        // 声明模式（never | once | always）：非法枚举由 setDisclaimerMode 抛错 → bad-request。
        if (!setDisclaimerMode) return fail('bad-request', '声明模式设置不可用 | disclaimer mode unavailable');
        try {
          setDisclaimerMode(payload?.mode);
        } catch (err) {
          return fail('bad-request', err?.message ?? String(err));
        }
        return await statusPayload();
      ...
      // 这些 endpoint 本身由 dsh 的浏览器认证栅栏（connection.requestRejection）把守，
      // 手机侧还额外经过 pocket 密码（代理在转发前就 401 了）。
      if (endpoint === POCKET_ENDPOINTS.devicesList) {
        if (!devices) return fail('bad-request', '设备管理不可用 | device management unavailable');
        return ok(devicesPayload());
      }
      if (endpoint === POCKET_ENDPOINTS.devicesRename) {
        if (!devices) return fail('bad-request', '设备管理不可用 | device management unavailable');
        const id = String(payload?.id ?? '');
        if (id === LEGACY_DEVICE_ID) return fail('bad-request', '旧设备不支持改名 | legacy device cannot be renamed');
        if (!devices.has(id)) return fail('bad-request', '设备不存在（可能已被下线）| device not found');
        try {
          devices.rename(id, payload?.name ?? '');
        } catch (err) {
          return fail('bad-request', err?.message ?? String(err));
        }
        return ok(devicesPayload());
      }
      if (endpoint === POCKET_ENDPOINTS.devicesRevoke) {
        if (!devices) return fail('bad-request', '设备管理不可用 | device management unavailable');
        const id = String(payload?.id ?? '');
        // 旧版凭证没有个体身份（所有 v1 设备同 cookie）→ 只能整体失效
        if (id === LEGACY_DEVICE_ID) {
          return fail('bad-request', '旧设备无法单独下线，请用「下线其他设备」| legacy device can only be revoked via "sign out other devices"');
        }
        if (!devices.revoke(id)) return fail('bad-request', '设备不存在（可能已被下线）| device not found');
        return ok(devicesPayload());
      }
      if (endpoint === POCKET_ENDPOINTS.devicesRevokeOthers) {
        if (!devices) return fail('bad-request', '设备管理不可用 | device management unavailable');
        const cur = currentDevice();
        // 调用者身份识别不出来（例如在电脑浏览器上没走过 pocket 密码）→ 谁都不保留：
        // 等价于「全部设备下线」，结果里如实告诉前端 keptCurrent=false。
        const keep = cur && !cur.legacy ? cur.id : null;
        const r = devices.revokeOthers(keep);
        // epoch 变了 → 调用者手里那枚 cookie 的 hash 已经对不上（epoch 在公式里），
        // 必须当场给他换一枚新 epoch 的 cookie，否则「下线其他设备」会把自己也踢掉。
        // 只有识别出身份、且设备仍在表里时才换（legacy / 未识别 → 本来就要重新登录）。
        const fresh = keep && typeof refreshCallerCookie === 'function' ? refreshCallerCookie(keep) : null;
        if (fresh && extra?.headers) extra.headers['set-cookie'] = fresh;
        return ok({ ...devicesPayload(keep), epoch: r.epoch, keptCurrent: keep !== null, removedLegacy: r.removedLegacy, cookieRefreshed: Boolean(fresh) });
      }
      // ---------- Web Push 通知（R3：公钥 / 订阅 / 退订 / 事件开关 / 测试推送） ----------
      // 写入对象一律是**调用者这台设备**（按请求 cookie 识别，与 devices.* 同一套身份判定）；
      // legacy 设备（升级前的 v1 凭证，无个体身份）不允许订阅——它随时可能被 epoch 清掉。
      /** 本机设备 id（v2 身份；识别不出 / legacy 返回 null）。 */
      const currentDeviceId = () => {
        const cur = currentDevice();
        return cur && !cur.legacy ? cur.id : null;
      };
      const notifyUnavailable = () => fail('bad-request', '推送通知不可用 | push notifications unavailable');
      if (endpoint === POCKET_ENDPOINTS.notifyVapidKey) {
        if (!notify) return notifyUnavailable();
        try {
          return ok({ key: notify.vapidKey() });
        } catch (err) {
          return fail('bad-request', `推送公钥不可用 | VAPID key unavailable: ${err?.message ?? err}`);
        }
      }
      if (endpoint === POCKET_ENDPOINTS.notifySubscribe) {
        if (!devices || !notify) return notifyUnavailable();
        const id = currentDeviceId();
        if (!id) return fail('bad-request', '无法识别本机设备身份，请在手机访问入口登录后再开启通知 | current device not identified');
        if (!devices.has(id)) return fail('bad-request', '设备不存在（可能已被下线）| device not found');
        const sub = payload?.subscription ?? null;
        const events = payload?.events && typeof payload.events === 'object' ? payload.events : null;
        const d = devices.setPush(id, sub, { events });
        if (!d) return fail('bad-request', '推送订阅数据非法（endpoint / p256dh / auth 缺失或格式不对）| invalid push subscription');
        return ok(devicesPayload());
      }
      if (endpoint === POCKET_ENDPOINTS.notifyUnsubscribe) {
        if (!devices) return notifyUnavailable();
        const id = currentDeviceId();
        if (!id) return fail('bad-request', '无法识别本机设备身份 | current device not identified');
        devices.clearPush(id);
        return ok(devicesPayload());
      }
      if (endpoint === POCKET_ENDPOINTS.notifySetEvents) {
        if (!devices) return notifyUnavailable();
        const id = currentDeviceId();
        if (!id) return fail('bad-request', '无法识别本机设备身份 | current device not identified');
        const d = devices.setPushEvents(id, payload?.events ?? {});
        if (!d) return fail('bad-request', '本机还没有推送订阅 | no push subscription on this device');
        return ok(devicesPayload());
      }
      if (endpoint === POCKET_ENDPOINTS.notifyTest) {
        if (!devices || !notify) return notifyUnavailable();
        const id = currentDeviceId();
        if (!id) return fail('bad-request', '无法识别本机设备身份 | current device not identified');
        if (!devices.get(id)?.push) return fail('bad-request', '本机还没有推送订阅 | no push subscription on this device');
        // 真实发一条：等结果回给 UI（失败也只回 ok:false 的明细，不抛）
        const r = await notify.test(id);
        return ok({ sent: r?.ok === true, status: r?.status ?? 0, dead: r?.dead === true, error: r?.error ?? null });
      }
